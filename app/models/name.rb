...
def self.search(search)
  if search
    where('cognome_nome LIKE ? or luogo_nascita LIKE ? or data_nascita LIKE ? or residenza LIKE ? or cittadinanza LIKE ? or ambits.ambito_criminale LIKE ? or areas.area_criminale LIKE ? or associations.associazione_criminale LIKE ?', "%#{search}%","%#{search}%", "%#{search}%", "%#{search}%","%#{search}%","%#{search}%","%#{search}%", "%#{search}%").includes(:ambit, :area, :association)
  else
    scoped
  end
end
